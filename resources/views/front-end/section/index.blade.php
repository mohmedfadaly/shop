...
<div class="felt">
  <div class="container">
    ...
    <div class="col-sm-12 ser">
       <form class="form-group" action="{{ route('front.section' , ['id' => $section->id]) }}">
        <input class="form-control" name="search" type="search" placeholder="ابحث هنا" aria-label="Search">
  ...
  <div class="header">
    ...
    @if(request()->has('search') && request()->get('search') != '')
                    <p style="margin-top: 10px">
                        you are search on  <b>{{ request()->get('search') }}</b> |  <a href="{{ route('front.section' , ['id' => $section->id]) }}"> Reset </a>
                    </p>
                @endif
...
$.each( providers, function( index, value ){
  var url = '{{ route("front.provider", "%id%") }}';
  url = url.replace('%id%', value.id);
    mymap.addMarker({
      ...
      title: value.name,
      infoWindow: {
          content: '<a href="'+url+'">' +value.name+ '</a>'
        }
    });
